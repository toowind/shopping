...
class Wechat
{
    ...
    const LOGIN_PREFIX = 'https://open.weixin.qq.com';
    const LOGIN_URL = '/connect/qrconnect?';
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function getLoginRedirect($callback,$state='',$scope='snsapi_login'){
        return self::LOGIN_PREFIX.self::LOGIN_URL.'appid='.$this->appid.'&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state.'#wechat_redirect';
    }
	/**
