<?php
namespace Taobao\Service\Product;
...
use Taobao\Service\Common;
use Taobao\Common\ApiException;
use Think\Log;
use Taobao\Common\Redis;
...
//                            echo $item['goods_id'].PHP_EOL;
    ...
    public static function getGoodsListByKeyword($paramData = array()){
        ...
        //page默认1
        if(isset($paramData["page"])) $RequestData["page_no"] = $paramData["page"];
...
//            );
        ...
        $rdata = self::http_get(self::$url.'/tbk/super_search',$RequestData);
...
//        $catesName = ['0_200'=>'热卖','0_1'=>'精选','0_2'=>'大咖推荐','0_10'=>'9.9专区','0_25'=>'生活超市','0_27'=>'居家日用','0_26'=>'母婴','0_22'=>'爆品'];
        ...
        $RequestData["apikey"] = 'fEvqmZ71kw43vkjV0SBZvrhdMHPsIEnv';
        $RequestData["sale_type"] =  2;
        $rdata = self::http_get(self::$url.'/spk/top',$RequestData);
        $data = json_decode($rdata,true);
//        var_dump($data);
        if($data["code"] != 200){
            Log::write($data,'ERROR');
...
VALUES";
        ...
        if (count($goodsList)) {
            foreach ($goodsList as $item) {
                $source_id = 2;
                $bar_code = $item['itemid'];
                $store_name = $item['itemtitle'];
                $brand_name = $item['shopname'];
                $shop_id = $item['shopid'];

                $image = $item['itempic'];
                $imageinfo = $item['taobao_image'];
                $cate_id = '热卖';
                $cate_id_no = '0_200';
                $keyword = '';
                $ot_price = $item['itemprice'];
                $price = $item['itemendprice'];
                $pingouPrice = 0;


                $coupon_discount = $item['couponmoney'];
                $commission_share = $item['tkrates'];
                $commission = $item['tkmoney'];
                $is_pg = 0;
                $is_coupon = $item['couponmoney'] ? 1:0;
                $order_count_30days = $item['itemsale'];
                $comments = 0;
                $goods_comments_share = 0;
                $is_show = 1;
...
//                continue;
//            }
            $ResponseData[$key]["goods_id"] = isset($val["item_id"]) ? $val["item_id"] : 0;
            $ResponseData[$key]["goods_name"] = isset($val["title"]) ? $val["title"] : "";
            $ResponseData[$key]["goods_thumbnail_url"] = isset($val["pict_url"]) ? $val["pict_url"] : "";
            ...
            //价格
            $min_group_price = isset($val["reserve_price"]) ? $val["reserve_price"] : 0;
            $ResponseData[$key]["min_group_price"] = $min_group_price;

            //京东佣金比例 百分比
            $promotion_rate = bcdiv($val["commission_rate"],10000,2);
            $ResponseData[$key]["promotion_rate"] = $promotion_rate;
            //京东佣金
            //单价  减去 优惠券
            $ResponseData[$key]["present_price"] = isset($val["zk_final_price"]) ? $val["zk_final_price"] : 0;
            $ResponseData[$key]["isCoupon"] = $val["coupon_amount"] ? 1 : 0;
            $ResponseData[$key]["isPg"] = $val["jdd_price"] ? 1 : 0;
            $ResponseData[$key]["discountPrice"] = isset($val["zk_final_price"]) ? $val["zk_final_price"] : 0;
            $ResponseData[$key]["pingouPrice"] = isset($val["jdd_price"]) ? $val["jdd_price"] : 0;
            $commission = bcmul($val["zk_final_price"],$promotion_rate, 2);
