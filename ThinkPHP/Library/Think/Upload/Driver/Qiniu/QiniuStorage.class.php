<?php
	...
	class QiniuStorage {
		...
		public $QINIU_FETCH_HOST = 'http://iovip.qbox.me/fetch';
		public $timeout = '';
		 ...
		 */
		public function fetchUrl($source,$file){
	
			$fetch = self::Qiniu_Encode($source);  
			//$to = self::Qiniu_Encode('zhijianyuan:20.jpg');  
			$to = self::Qiniu_Encode("{$this->bucket}:{$file}");

			$url  = "{$this->QINIU_FETCH_HOST}/". $fetch .'/to/' . $to;  
