...
class Dispatcher
{
    ...
    public static function dispatch()
    {
        ...
        define("__APPCONF__",ROOT_PATH."Application".$depr.str_replace("/","",__MODULE__).$depr.'Conf'.$depr.'app.ini');
        define("__APPROUTE__",ROOT_PATH."Application".$depr.str_replace("/","",__MODULE__).$depr.'Conf'.$depr.'route.ini');
        // 获取控制器和操作名
...
//            return true;
//        }
//        $routeConf = MODULE_PATH.'Conf/route.ini';
//        if(!is_file($routeConf)){
//            mkdir($routeConf, 0755, true);
//        }
//
//        $routeConfig = parse_ini_file(__APPROUTE__,true);
//        if(empty($routeConfig)){
//            exit(json_encode(array("code"=>1,"msg"=>"Route.ini文件为空","data"=>array())));
//        }
//        $routeConfig = self::Route($routeConfig,ACTION_NAME);
//
//        if($routeConfig == false){
//             exit(json_encode(array("code"=>1,"msg"=>"方法不存在","data"=>array())));
//        }
    }
...
//            if($module != NULL && isset($_REQUEST["method"])){
//                $module = explode("_",$_REQUEST["method"])[0];
//            }else if($module != NULL && !isset($_REQUEST["method"])){
//                $module = $module;
//            }else if($module == NULL && !isset($_REQUEST["method"])){
//                $module = C('DEFAULT_MODULE');
//            } else{
...
//            }
    ...
    public static function Route($routeConfig,$actionName){
        $Config = array();
        if(empty($routeConfig)){
            return $Config;
        }
        $moduel = 'method.'.$actionName.'.Module';
        if(!isset($moduel,$routeConfig)){
            return false;
        }

        $Controller = 'method.'.$actionName.'.Controller';
        if(!isset($Controller,$routeConfig)){
            return false;
        }

        $action = 'method.'.$actionName.'.Action';
        if(!isset($action,$routeConfig)){
            return false;
        }
        return true;
    }
