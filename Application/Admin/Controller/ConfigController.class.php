...
class ConfigController extends AdminController {
    ...
    public function save($config){
        if($config && is_array($config)){
            ...
            if(!empty($config["LOWER_SHELF_GOODS_ID_LIST"])){
                //清除缓存
                $LOWER_SHELF_GOODS_ID_LIST = explode(',',$config["LOWER_SHELF_GOODS_ID_LIST"]);
                 $redis = new \Common\Common\Cache\Redis(C("redis"));
                 $redis->del("lower_shelf_goods_id_list");
                 foreach ($LOWER_SHELF_GOODS_ID_LIST as $val){
                     $redis->SADD("lower_shelf_goods_id_list",$val);
                 }
            }
        }
        S('DB_CONFIG_DATA',null);
    ...
    public function group() {
        ...
        $list   =   M("Config")->where(array('status'=>1,'group'=>$id))->field('id,name,title,extra,value,remark,type,group')->order('sort')->select();
        //秒杀下架商品id读取缓存
        if($id ==6){
            $newList = array_column($list,NULL,"name");
            $redis = new \Common\Common\Cache\Redis(C("redis"));
            $goodsIdList = $redis->SMEMBERS("lower_shelf_goods_id_list");
            $newList["LOWER_SHELF_GOODS_ID_LIST"]["value"] = implode(",",$goodsIdList);
        }else{
            $newList = $list;
        }
        if($list) {
