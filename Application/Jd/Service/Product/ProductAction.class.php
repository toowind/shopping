...
//            }
            ...
            //京东佣金
            //单价  减去 优惠券
            $ResponseData[$key]["present_price"] = isset($val['pinGouInfo']['pingouPrice']) ? ($val['pinGouInfo']['pingouPrice']-$coupon_discount) : ($min_group_price-$coupon_discount);
            $ResponseData[$key]["isCoupon"] = $ret[0];
            $ResponseData[$key]["isPg"] = isset($val["pinGouInfo"]["pingouPrice"]) ? ($val["pinGouInfo"]["pingouPrice"]>0) : 0;
            $ResponseData[$key]["discountPrice"] = isset($val['pinGouInfo']['pingouPrice']) ? ($val['pinGouInfo']['pingouPrice']-$coupon_discount) : ($min_group_price-$coupon_discount);
...
//            $ResponseData[$key]["return_cash"] = bcmul($commission,0.1, 2);
            ...
            $ResponseData[$key]["source_commission_share"] = $val["commissionInfo"]["commissionShare"];
            $ResponseData[$key]["source_commission"] = $val["commissionInfo"]["commission"];
    ...
    public static function getProductInfo($data,$type = 0){
        ...
        if(!empty($goodsInfo) && !is_null($ResponseData['goods_id']) && count($ResponseData['goods_gallery_urls'])){
            ...
            if($mysqli){
                ...
                $ResponseData["coupon_discount"] = $coupon_discount;
                $ResponseData["return_cash_total"] = $ResponseData["return_cash"] = bcmul($min_group_price-$coupon_discount, $promotion_rate, 2);
